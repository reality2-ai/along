...
try{
 ...
 await page.goto(base+'install.html');await expect(page.locator('h1')).toContainText('offline');await expect(page.locator('main')).toContainText('It does not show live bus, train or ferry positions');expect(errors).toEqual([]);
 console.log(JSON.stringify({url:base,appVersion:27,workerScope:scope,installability:true,settingsAboutLink:true,offlineNewAddressJourney:true,offlineInstallGuide:true,pageErrors:errors},null,2));
