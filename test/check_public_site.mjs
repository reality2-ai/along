...
try{
 ...
 await page.locator('#settings-open').click();await page.locator('#language-choice').selectOption('mi');await expect(page.locator('#language-draft')).toContainText('generated by AI and may contain mistakes');await page.locator('#settings .close-dialog').click();
 await expect(page.locator('#flow-title')).toHaveText('Kōwhiria tō haerenga');await expect(page.locator('.journey-card').first()).toBeVisible();
 await page.reload();await expect(page.locator('#flow-title')).toHaveText('Kei te hiahia haere koe ki hea?');await expect(page.locator('#language-notice')).toBeVisible();
 await page.goto(base+'install.html');await expect(page.locator('h1:visible')).toContainText('Tāutahia');await page.locator('#guide-language-choice').selectOption('en');await expect(page.locator('h1:visible')).toContainText('offline');await expect(page.locator('main')).toContainText('It does not show live bus, train or ferry positions');expect(errors).toEqual([]);
 console.log(JSON.stringify({url:base,appVersion:30,workerScope:scope,installability:true,settingsAboutLink:true,offlineNewAddressJourney:true,offlineInstallGuide:true,draftLanguageOffline:true,pageErrors:errors},null,2));
