...
const version=process.env.EXPECT_APP_VERSION||'44';
...
try{
 ...
 await expect(page).toHaveTitle('Along · Auckland public transport');await expect(page.locator('#app-purpose')).toBeVisible();await expect(page.locator('.flow-nav')).toBeHidden();
 ...
 await expect(page.locator('#journey-notes')).not.toHaveAttribute('open','');await page.locator('#journey-feedback').click();
 ...
 await page.goto(base+'install.html');await expect(page.locator('h1')).toContainText('offline');await expect(page.locator('#guide-language-choice')).toHaveCount(0);await expect(page.locator('main')).toContainText('By default, departures and journey times come from the downloaded timetable.');expect(errors).toEqual([]);
 console.log(JSON.stringify({url:base,appVersion:version,workerScope:scope,installability:true,settingsAboutLink:true,offlineNewAddressJourney:true,offlineInstallGuide:true,englishOnly:true,offlineFeedbackDraft:true,pageErrors:errors},null,2));
