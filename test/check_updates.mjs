...
try{
 ...
 assert.match(await page.locator('#settings').textContent(),/App version 9/);
 assert.equal(await page.evaluate(()=>localStorage.getItem('update-check')),'saved');
 // Recovery ignores a previously saved Māori choice, and remains functional
 ...
 await reo.waitForFunction(()=>document.querySelector('h1').textContent==='Update Along');
 assert.equal(await reo.locator('#recovery-draft').count(),0);
 await reo.locator('#recover-update').click();
 await reo.waitForFunction(()=>document.getElementById('recovery-status').textContent==='Installed version 11.',{timeout:45000});
 assert.equal(await reo.locator('#recover-update').textContent(),'Open Along →');
 ...
 console.log('PASS: recovery stays English despite a saved Māori preference; missing phrase module leaves recovery operational.');
