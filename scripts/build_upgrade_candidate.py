...
def candidate(runtime):
    ...
    with tempfile.TemporaryDirectory(dir=output.parent) as temporary:
        ...
        for name, expected in original['files'].items():
            ...
            if target.suffix in ('.js', '.mjs', '.html', '.css'):
                ...
                if name == 'public/sw.js':
                    text = text.replace('along-experimental-shell-', 'along-shell-').replace('along-shell-v37', 'along-shell-v43')
                if name == 'public/index.html':
                    text = text.replace('App version 37', 'App version 43')
        ...
        (stage / 'build-info.json').write_text(json.dumps({
            'profile': PROFILE, 'appVersion': '43', 'publishable': False,
