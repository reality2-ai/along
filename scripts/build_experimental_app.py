...
"""
        ...
        if preview:
            ...
            index.write_text(re.sub(r'App version \d+', 'App version 3806 · Device preview', index.read_text())
            ...
            worker.write_text(re.sub(r"(const CACHE = 'along-device-preview-shell-v)\d+", r'\g<1>3806', worker.read_text()))
            update = stage / 'public/update.html'
            update.write_text(re.sub(r'(recovery=|Recovery page )\d+', r'\g<1>3806', update.read_text()))
        ...
        (stage / 'build-info.json').write_text(json.dumps({'profile': profile, 'files': files,
            **({'namespaces': namespaces, 'appVersion': '3806'} if preview else {})}, indent=2) + '\n')
        if output.exists():
