...
const arrive=process.argv.includes('--arrive-by');
...
const report={timeMode:arrive?'arrive':'leave',date:'2026-09-23',runtime:process.version,loadMs:Math.round(performance.now()-start),cases:[]};
for(const [name,fromQuery,toQuery,modes] of [
 ...
 const began=performance.now();const journeys=planner.plan({from,to,date:report.date,time:'09:00',timeMode:report.timeMode,modes});
 if(!journeys.length)throw new Error(`No journey: ${name}`);
 const journey=journeys[0];
 if(arrive&&journeys.some(j=>j.arrival>32400))throw new Error('Arrival deadline missed');
...
await mkdir('test-results',{recursive:true});await writeFile(arrive?'test-results/real-journeys-arrive.json':'test-results/real-journeys.json',JSON.stringify(report,null,2)+'\n');
