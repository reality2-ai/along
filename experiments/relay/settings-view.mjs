...
export function showRelaySettings(container,{wasm,store,expectedGroup,member,focus=false,onBack=()=>{},onChanged=()=>{}}){
  ...
  const details=node('details','');details.append(node('summary','What does the relay see?'),node('p','The relay sees your network address, group and device identifiers, membership certificates, and connection timing and traffic sizes. Shared saved journeys are encrypted between permitted devices. Your AT key, learning history and current location are not sent by this connection.'),node('p','This uses Along’s browser-only R2 subset. It does not provide hardware-backed key storage. A relay cannot pair devices or grant sharing permission. Devices at different recovery checkpoints still need a checkpoint transfer and review. Mobile browsers may suspend Along in the background.'));
