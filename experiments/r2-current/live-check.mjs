...
// Usage: node experiments/r2-current/live-check.mjs [url] [evidence.json]
...
const url = process.argv[2] ?? 'wss://wairoa.mariko.org.nz/r2';
...
const result = {
  ...
  scope: 'Two Node WebSocket clients with a synthetic group created for this run exchanged XChaCha20-Poly1305/HMAC-SHA256 protected extended EVENT frames through the deployed host. This verifies Along\'s current-frame codec and protection against the real relay. It does not verify the browser app, saved-journey synchronisation, enrolment, revocation, reconnection or physical devices.',
};
console.log(JSON.stringify(result, null, 2));
if (out) writeFileSync(out, JSON.stringify(result, null, 2) + '\n');
process.exit(forward && reverse && strangerArrived && !strangerDelivered ? 0 : 1);
