...
export function showStopLiveUpdates(container, {client, place, rows, at, now}) {
  if (!Array.isArray(rows)) throw new Error('Stop context unavailable');
  const context = structuredClone({place, rows, at});
  context.rows = context.rows.filter(row => row.stop?.id === context.place?.id);
  return showContextLiveUpdates(container, {client, now, date: context.at.date,
    rows: context.rows.map(row => ({...row, description: `${row.route} to ${row.headsign}`})),
    contexts: stopAlertContexts(context.place, context.rows, context.at)});
}

// Shared display lifecycle; wrappers construct the verified stop/leg scope.
export function showContextLiveUpdates(container, {client, rows, contexts, date,
  scope = 'this stop', scheduledText = 'Scheduled times', enabled = true,
  now = () => Date.now() / 1000}) {
  if (typeof client?.read !== 'function' || typeof client?.cancel !== 'function') throw new Error('Live client unavailable');
  const context = structuredClone({rows, contexts, date});
  ...
  const button = element('button', 'Check live times and alerts'); button.hidden = !enabled; button.type = 'button'; button.className = 'pairing-primary';
  const help = element('p', `Optional online check for ${scope}. Your saved key goes directly to Auckland Transport; the stop and journey details stay on this device. ${scheduledText} remain${scheduledText === 'Scheduled times' ? '' : 's'} available.`);
  const status = element('p', enabled ? '' : 'No remaining public-transport legs to check.'); status.setAttribute('role', 'status');
  const results = element('div', '');
  panel.append(element('h2', `Live updates for ${scope}`), button, help, status, results); container.replaceChildren(panel);
  const dispose = () => {
    if (disposed) return;
  ...
  button.addEventListener('click', async event => {
    if (!event.isTrusted || disposed || busy || !enabled) return;
    busy = true; button.disabled = true; clearTimeout(timer); results.replaceChildren();
    status.textContent = `Checking live information for ${scope}…`;
    try {
      ...
      for (const row of context.rows) {
        ...
        if (prediction.status === 'predicted') {
          const scheduled = aucklandWallEpoch(context.date, row.departure);
        ...
        const scheduled = aucklandWallEpoch(context.date, row.departure);
        const item = element('p', `${row.description}: ${label}. Scheduled ${scheduled === null ? 'time unavailable' : time.format(new Date(scheduled * 1000))}.`);
        results.append(item); matches++;
        expires = Math.min(expires, Number(prediction.updated) + 180);
      }
      const relevant = contextualAlerts(alerts, context.contexts, {now: checkedAt});
      if (relevant.length) {
        results.append(element('h3', `Service updates for ${scope}`));
      ...
      status.textContent = matches || relevant.length
        ...
        : `No matching live information is available. ${scheduledText} remain${scheduledText === 'Scheduled times' ? '' : 's'} unchanged.`;
      if (Number.isFinite(expires)) timer = setTimeout(() => {
        if (!disposed) { results.replaceChildren(); status.textContent = `Live information has expired. ${scheduledText} remain${scheduledText === 'Scheduled times' ? '' : 's'} available; check again when needed.`; }
      }, Math.max(0, (expires - now()) * 1000));
    } catch {
      if (!disposed) { results.replaceChildren(); status.textContent = `Live information is unavailable. ${scheduledText} remain${scheduledText === 'Scheduled times' ? '' : 's'} unchanged.`; }
