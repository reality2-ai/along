...
try {
  ...
  if(process.env.ENROLLED_RELAY==='1'||(relayNetwork&&!relayGeneration)){
    ...
    if(relayNetwork){assert.equal(await page.evaluate(()=>relayServicePassed),true);const stats=relayServer.stats();assert.ok(stats.connections>=3);assert.ok(stats.frames>10);assert.equal(stats.plaintextObserved,false);if(!process.env.R2_HIVE_UPSTREAM)assert.equal(stats.limited,0,'Connection replacement must preserve the origin rate window');console.log('Relay pacing check: '+JSON.stringify({connections:stats.connections,frames:stats.frames,limited:stats.limited}));}
