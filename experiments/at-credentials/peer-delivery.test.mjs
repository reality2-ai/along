...
for (const name of ['app-preferences.mjs','preference-envelope.mjs','isolated-preferences.mjs','generation-state.mjs','generation-migration.mjs','generation-checkpoint.mjs','checkpoint-preparation.mjs','checkpoint-installation.mjs','checkpoint-review.mjs','checkpoint-choice-commit.mjs','startup-state.mjs','older-edit-progress.mjs','migration-setup.mjs','checkpoint-permission.mjs','checkpoint-inbox.mjs','checkpoint-exchange.mjs','checkpoint-session.mjs','checkpoint-selection.mjs','generation-store.mjs','generation-app-store.mjs','generation-exchange.mjs','generation-permission.mjs','enrolled-checkpoint.test.mjs']) sources.set('/'+name,await readFile(new URL('../journey-sync/'+name,import.meta.url)));
...
for (const name of ['hive-relay-transport.mjs','hive-transport.mjs','group-protection.mjs','frame.mjs','heartbeat.mjs','duplicates.mjs','names.mjs','segments.mjs','cbor.mjs']) sources.set('/'+name,await readFile(new URL('../r2-current/'+name,import.meta.url)));
...
try {
  ...
  await page.evaluate(async () => {
    ...
    if(globalThis.checkEnrolledRelay) {
      ...
      if(globalThis.relayNetwork)await (await import('./service-check.mjs')).checkRelayService({wasm,owner,receiver,group});
  ...
  if(process.env.ENROLLED_RELAY==='1'||(relayNetwork&&!relayGeneration)){
    ...
    if(relayNetwork){assert.equal(await page.evaluate(()=>relayServicePassed),true);const stats=relayServer.stats();assert.ok(stats.connections>=3);assert.ok(stats.frames>10);assert.equal(stats.plaintextObserved,false);}
    console.log('PASS: real enrolled relay handshake and snapshot convergence, offline edit/reconnect, committed receipts, consent removal, regrant invalidation and identity/certificate refusal; '+(relayNetwork?'real local WSS, current hive binding stand-in with group-protected extended frames, no fixture journey labels observed in forwarded bytes.':'controlled relay carriage, not WSS.'));
  }
  if(relayGeneration){assert.equal(await page.evaluate(()=>generationRelayPassed),true);assert.ok(relayServer.stats().connections>=3);console.log('PASS: generation-two snapshots over enrolled WSS, explicit planner reconciliation/history preservation, offline reconnect and generation advance refusal.');}
  ...
  if(process.env.ENROLLED_CHECKPOINT==='1'||relayGeneration) {
    ...
    console.log('PASS: enrolled-device checkpoint consent, permission-removal race, guarded installation/retry and local-difference application; recovered independent save/history reopen in a fresh page. Checkpoint transfer/retry and generation-two snapshot convergence use authenticated WebRTC; unfinished review, consent races and generation changes refuse access. Harness signaling on one host; installation fault checks use direct adapter calls.');
  }
