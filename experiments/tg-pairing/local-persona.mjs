...
export async function loadLocalPersona({wasm, store, expectedGroup}) {
  ...
  if (value.format !== 1 || value.claim !== 'owner' || record?.format !== 1
      ...
      || !bytes(value.invitation?.group, 32) || !equal(value.invitation.group, group)
  ...
  const journalKey = hex(group) + ':' + hex(value.invitation.code);
  const journal = await store.read('enrollment-invitations', journalKey);
  if (journal?.value?.format !== 1 || journal.value.state !== 'consumed') throw refuse();
  const current = async () => {
    const latest = await store.read('candidate-persona', 'active');
    const used = await store.read('enrollment-invitations', journalKey);
