...
const recoveryMode = process.env.AUTOMATIC_RECOVERY === '1' || process.env.GUIDED_RECOVERY === '1';
...
if (recoveryMode) for (const name of ['automatic-epoch-recovery-view.mjs','epoch-recovery-view.mjs','automatic-epoch-recovery.mjs','epoch-transition.mjs','epoch-preparation.mjs','epoch-recovery-material.mjs','epoch-installation.mjs','epoch-recovery-proof.mjs','epoch-recovery-session.mjs','epoch-recovery-receipt.mjs','member-removal.mjs','removal-set.mjs']) sources.set('/'+name,await readFile(new URL('./'+name,import.meta.url)));
...
try {
  ...
  if(recoveryMode) {
    await checkAutomaticEpochRecovery(pages,`wss://127.0.0.1:${server.address().port}/r2`,{guided:process.env.GUIDED_RECOVERY==='1',stats:()=>relay.stats()});
