...
export function showMemberRemoval(container, {wasm, store, expectedGroup, subject, certificate,
  ...
  const explanation = node('p', 'This saves a group removal on this device. Other devices must receive the removal before they can enforce it. Delivery and group-key replacement are not available in this preview.');
  ...
  const dispose = () => {
    if (disposed) return; disposed = true; lifetime.abort(); remove.disabled = true; back.disabled = true;
