...
export function showReceiveInvitation(container, {focus = false, onBack = () => {}} = {}) {
  ...
  panel.append(heading, explanation, label, status, next, back); container.replaceChildren(panel);
  if (focus) heading.focus();
  const dispose = () => {
    if (disposed) return;
    disposed = true; lifetime.abort(); reviewed = undefined; input.value = '';
  ...
  const goBack = () => {
    ...
    if (reviewed && !accepted) {
      reviewed = undefined; label.hidden = false;
  ...
  next.addEventListener('click', event => {
    ...
    if (!reviewed) {
      try {
        ...
        label.hidden = true; heading.textContent = 'Is this from your other device?';
