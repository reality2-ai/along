...
if (process.env.EPOCH_INSTALL === '1') {
  ...
  for (const name of ['epoch-transition.mjs', 'epoch-preparation.mjs', 'epoch-recovery-material.mjs', 'epoch-installation.mjs', 'epoch-install-check.mjs', 'epoch-recovery-proof.mjs', 'epoch-recovery-session.mjs', 'member-removal.mjs']) sources.set('/' + name, await readFile(new URL(name, import.meta.url)));
}
...
try {
  ...
  if (process.env.EPOCH_INSTALL === '1') {
    ...
    console.log('PASS: older-device recovery proof checks nonce replay, cancellation, expiry and removal; real WebRTC recovery mutually authenticates epoch-one/two devices, refuses a forged issuer proof and closes on removal. Harness transfers SDP; no recovery keys are delivered.');
