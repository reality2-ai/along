...
export function showComparison(container, {code, onDecision, signal, focus = false}) {
  ...
  const expire = () => {
    if (disposed) return;
    disposed = true; match.disabled = true; cancel.disabled = true;
  ...
  return Object.freeze({dispose: () => { expire(); signal?.removeEventListener('abort', expire); }});
}
