...
for (const name of ['enrollment-profile.mjs', 'enrollment-payloads.mjs', 'core-candidate-session.mjs', 'software-traffic.mjs', 'stored-claim.mjs', 'installation-receipt.mjs']) sources.set('/' + name, await readFile(new URL('./' + name, import.meta.url)));
