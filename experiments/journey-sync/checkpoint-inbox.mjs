...
export async function retainJourneyCheckpoint({store,expectedGroup,checkpoint,snapshot,signal}) {
  ...
  for(let attempt=0;attempt<8;attempt++){
    ...
    const checks=[{scope:replicas,key:group,expectedRevision:replica.revision}];
    if(saved){
      const value=saved.value;
      if(!value||Object.keys(value).sort().join(',')!=='checkpoint,format,previous,snapshot'
          ||value.format!==1||!(value.checkpoint instanceof Uint8Array))throw fail();
      ...
      if(hex(value.checkpoint)===hex(bytes)){
        await verifyJourneyCheckpoint({bytes,current:value.previous,snapshot:copy});
      ...
      // Advance the single slot only after its exact signed bundle has been
      // installed and retained in recovery. Local review may still be pending;
      // that recovery record stays intact and the next installation must wait.
      if(current.generation!==next.generation||current.checkpoint!==next.checkpoint)throw fail();
      const archiveScope='along-journey-checkpoint-recovery-v1',key=group+':'+next.generation;
      const archive=await store.read(archiveScope,key),recovery=archive?.value;
      if(recovery?.format!==1||!(recovery.checkpoint instanceof Uint8Array)
          ||hex(recovery.checkpoint)!==hex(value.checkpoint))throw fail();
      const archived=await verifyJourneyCheckpoint({bytes:recovery.checkpoint,current:recovery.previous,snapshot:recovery.snapshot});
      if(archived.checkpoint!==next.checkpoint||archived.generation!==next.generation)throw fail();
      checks.push({scope:archiveScope,key,expectedRevision:archive.revision});
    }
    await verifyJourneyCheckpoint({bytes,current,snapshot:copy});
    active();
    const result=await store.compareAndSwapMany([{scope,key:group,expectedRevision:saved?.revision??0,
