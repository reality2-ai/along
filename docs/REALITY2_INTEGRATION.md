...
application-payload or key-delivery method yet. Ordered delivery and receipt
recovery remain unfinished. See [the rotation design](R2_EPOCH_ROTATION.md#recovery-only-possession-check).
...
mocked AT reads and removal before further requests. The experimental app mounts
this flow in Settings. A generated-app test now passes across two isolated browser
...
approval and signaling remain harness inputs. The selected software profile now persists recipient traffic keys encrypted in the
same installation transaction and checks current membership when restoring them.
Epoch rotation and public setup wiring remain unfinished.
The actual enrollment path is now joined to the AT credential scenario. The next
integration work is invitation review and device signaling in the interface,
plus epoch rotation and catch-up. More downstream UI tests alone cannot establish that outcome.
