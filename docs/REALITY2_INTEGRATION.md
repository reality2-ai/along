...
| AT credential custody | Experimental own-member owner pin, signed device grants, encrypted IndexedDB record and direct-client access checks; combined consent/settings flow passes browser setup, save and restore checks | Public UI integration, authenticated delivery, cross-device freshness and installed-device provider verification |
