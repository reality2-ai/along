...
export async function writePlannerPreferences(data, storage, locks = globalThis.navigator?.locks) {
  ...
  try {
    ...
    if (before.sync) {
      ...
      return await locks.request('along-device-preview-journey-import:' + before.sync.group, () => {
    ...
    return await locks.request('along-device-preview-journey-import:local', () => {
...
export async function writePreferencesLocked(data, {expectedRaw, storage = selectPlannerStorage(),
  ...
  return locks.request('along-device-preview-journey-import:' + before.sync.group, {signal}, () => {
