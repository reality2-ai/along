...
export function openGenerationAppJourneyStore({store, group, actor, storage = selectPlannerStorage(), locks = navigator.locks}) {
  ...
  const reconcile = ({signal} = {}) => locks.request('along-device-preview-journey-import:' + group, {signal}, async () => {
    for (;;) {
      const olderReview=await store.read('along-older-edit-pending-v1',group);
      if(olderReview&&![2,3].includes(olderReview.value?.format))throw Error('Finish the retained older-copy review before sharing');
      if (signal?.aborted) throw Error('Sharing ended');
