  ...
  const home = async () => {
    ...
    try {
      if (!store) {
        ...
        const opened = await storage.openBrowserStorage('along-device-preview-devices-v1');
      ...
      if (!saved) {
        ...
        if (identity.origin === 'initial') {
          ...
          if (identity.epoch > 0n) action(details, 'Send a group key update', () => show(showMemberDevices, {databaseName: 'along-device-preview-devices-v1', purpose: 'update'}));
          action(details, 'Review group devices', () => show(showMemberDevices, {databaseName: 'along-device-preview-devices-v1'}));
