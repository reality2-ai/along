...
export function showMemberDevices(container, {wasm, store, expectedGroup, focus = false, onBack = () => {}}) {
  const group = expectedGroup.slice(), document = container.ownerDocument;
  let child, disposed = false, generation = 0;
  ...
  const home = async () => {
    ...
    const explanation = node('p', 'Choose a device to review its group removal. A saved certificate does not mean the device finished joining or is online. Devices added with older previews may be missing from this list.');
    const list = node('div', '');
